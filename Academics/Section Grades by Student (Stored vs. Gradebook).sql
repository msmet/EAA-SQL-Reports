SELECT
	...
	,	CASE
			...
			END
	,	CASE
			WHEN	((sect.ExcludeFromStoredGrades = 0 AND c.ExcludeFromStoredGrades = 1) OR sect.ExcludeFromStoredGrades = 1 AND sg.grade IS NULL)
					THEN	'Correct Excluded'
			WHEN	((sect.ExcludeFromStoredGrades = 0 AND c.ExcludeFromStoredGrades = 1) OR sect.ExcludeFromStoredGrades = 1 AND sg.grade IS NOT NULL)
					THEN	'Incorrect Not Excluded'
			WHEN	(sg.dcid IS NULL)
					THEN	'Incorrect Not Stored'
			WHEN	(sg.grade IS NULL OR sg.grade = '-'||'-')
					THEN	'Incorrect Missing Grade'
			WHEN	(sg.grade = pgfg.grade AND sg.percent = pgfg.percent)
					THEN	'Correct Grades Match'
			WHEN	(pgfg.dcid IS NULL)
					THEN	'Incorrect Current Grade Missing'
			ELSE	'Unknown'
			END
