SELECT
	s.dcid
	,	CASE WHEN MAX(PSER.grade_level) <= 8 THEN 'K-8' WHEN MAX(PSER.grade_level) >= 9 THEN '9-12' ELSE 'Unknown' END
	...
	,	CASE WHEN SUM (CASE WHEN PSER.entrydate <= '~[short.date]' AND PSER.exitdate > '~[short.date]' THEN 1 ELSE 0 END) > 0 THEN 'Active' ELSE 'Inactive' END
