...
(
	SELECT
		CASE WHEN sch.high_grade < 9 THEN 'K-8' WHEN sch.high_grade >= 9 THEN '9-12' ELSE 'Unknown' END AS school_group
...
SELECT
	CASE WHEN GROUPING(log_raw.school_group) = 1 THEN 'All' ELSE log_raw.school_group END
	,	CASE WHEN GROUPING(sch.name) = 1 THEN 'All' ELSE sch.name END
	,	CASE WHEN GROUPING(log_raw.grade_level) = 1 THEN 'All' ELSE TO_CHAR(log_raw.grade_level) END
	,	CASE WHEN GROUPING(log_raw.gender) = 1 THEN 'All' ELSE TO_CHAR(log_raw.gender) END
	,	CASE WHEN GROUPING(log_raw.special_program) = 1 THEN 'All' ELSE log_raw.special_program END
	,	CASE WHEN GROUPING(log_raw.state_ethnicity) = 1 THEN 'All' ELSE log_raw.state_ethnicity END
	,	COUNT (DISTINCT log_raw.studentid)
	,	COUNT (DISTINCT CASE WHEN log_raw.log_type = 'Discipline' THEN log_raw.logdcid ELSE NULL END)
	,	COUNT (DISTINCT CASE WHEN log_raw.expulsion_flag = 'Yes' THEN log_raw.logdcid ELSE NULL END)
	,	COUNT (DISTINCT CASE WHEN log_raw.oss_flag = 'Yes' THEN log_raw.logdcid ELSE NULL END)
	,	COUNT (DISTINCT CASE WHEN log_raw_b.oss_count IN (0,NULL) THEN log_raw_b.studentid ELSE NULL END)
	,	COUNT (DISTINCT CASE WHEN log_raw_b.oss_count = 1 THEN log_raw_b.studentid ELSE NULL END)
	,	COUNT (DISTINCT CASE WHEN log_raw_b.oss_count = 2 THEN log_raw_b.studentid ELSE NULL END)
	,	COUNT (DISTINCT CASE WHEN log_raw_b.oss_count = 3 THEN log_raw_b.studentid ELSE NULL END)
	,	COUNT (DISTINCT CASE WHEN log_raw_b.oss_count >= 4 THEN log_raw_b.studentid ELSE NULL END)
	,	CAST (AVG (log_raw.oss_exp_length) AS DECIMAL (4,1))
	...
	,	CAST (100*COUNT (DISTINCT CASE WHEN log_raw_b.oss_count > 1 THEN log_raw_b.studentid ELSE NULL END) / COUNT (DISTINCT log_raw.studentid) AS DECIMAL (5,1))
...
(
	SELECT
		studentid
		,	schoolid
		,	SUM (CASE WHEN oss_flag = 'Yes' THEN 1 ELSE 0 END) oss_count
	...
	GROUP BY
		...
		ON		log_raw_b.schoolid		=	log_raw.schoolid
		AND		log_raw_b.studentid		=	log_raw.studentid
		
WHERE
	sch.school_number NOT IN (45541,777777,999999,6666666,666666,11111)
	AND	log_raw.grade_level	>=	%param4%
	~[if.is.a.school]
		AND	sch.school_number			=	~(curschoolid)
	[/if]
GROUP BY
	GROUPING SETS
		(	(log_raw.school_group, sch.name)
		,	(log_raw.school_group, sch.name, log_raw.grade_level)
		,	(log_raw.school_group, sch.name, log_raw.gender)
		,	(log_raw.school_group, sch.name, log_raw.special_program)
		,	(log_raw.school_group, sch.name, log_raw.state_ethnicity)
		~[if.district.office]
			,	sch.Alternate_School_Number
			,	log_raw.school_group
			,	(log_raw.school_group, log_raw.grade_level)
			,	(log_raw.school_group, log_raw.gender)
			,	(log_raw.school_group, log_raw.special_program)
			,	(log_raw.school_group, log_raw.state_ethnicity)
			,	log_raw.gender
			,	log_raw.special_program
			,	log_raw.state_ethnicity
		[/if]
		)