...
(
	SELECT
		...
		,	PSER.studentid
		,	PSER.grade_level
		,	CASE WHEN (SELECT count(*) FROM PS_enrollment_reg PSERb WHERE PSERb.studentid = PSER.studentid AND PSERb.exitdate >= '%param3%' AND PSERb.entrydate < '%param4%' AND PSERb.entrydate != PSERb.exitdate) > 0  THEN 'Returning' ELSE 'New' END AS cat_returning
