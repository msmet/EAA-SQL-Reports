SELECT
	...
	,	s.lastfirst
	,	PSER.grade_level
	,	(SELECT LISTAGG (tea.lastfirst, ', ') WITHIN GROUP (ORDER BY tea.lastfirst) FROM cc INNER JOIN teachers tea  ON tea.id = cc.teacherid WHERE cc.studentid = PSER.studentid AND cc.dateenrolled <= '%param2%' AND cc.dateleft > '%param2%' AND cc.course_number IN ('HRAM','EM72106TYR','HS22106TSA'))
	,	s.street
	,	s.city
	,	s.state
	,	s.zip
...
HAVING
	...
	OR	CAST(COUNT ( DISTINCT ( CASE WHEN (PSER.grade_level <= 8 AND pam2.pres_count  > 0) THEN pam2.date_value WHEN (PSER.grade_level > 8 AND pam2.pres_count  > 1) THEN pam2.date_value ELSE NULL END))/COUNT( DISTINCT pam2.date_value) AS DECIMAL (4,1)) < %param5%
