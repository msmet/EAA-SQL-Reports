SELECT
	...
	,	CONCAT(CAST(cast(100*SUM(CASE WHEN pgfg.grade IN ('Cr','P') THEN 1 ELSE 0 END)/count(*) as decimal(4,1)) AS varchar(10)),'%') AS a_perc
	,	CONCAT(CAST(cast(100*SUM(CASE WHEN pgfg.grade IN ('NC','F') THEN 1 ELSE 0 END)/count(*) as decimal(4,1)) AS varchar(10)),'%') AS b_perc
	,	CONCAT(CAST(cast(100*SUM(CASE WHEN pgfg.grade NOT IN ('Cr','P','NC','F') THEN 1 ELSE 0 END)/count(*) as decimal(4,1)) AS varchar(10)),'%') AS OTHER_perc
